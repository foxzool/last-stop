...
use crate::bus_puzzle::{
    manhattan_distance, ConnectionType, GameStateEnum, GridPos, LevelManager, PathfindingGraph,
    RouteSegment, RouteSegmentType, StationEntity,
};
use bevy::prelude::*;
...
fn should_connect_station_segment(station_pos: GridPos, segment: &RouteSegment) -> bool {
    ...
    let connection_points =
        get_theoretical_connections(segment.grid_pos, &segment.segment_type, segment.rotation);
...
fn analyze_connection_failure(
    ...
    info!(
        "  距离: {}",
        manhattan_distance(station_pos, segment.grid_pos)
    );
    ...
    for (i, &connection_point) in connection_points.iter().enumerate() {
        ...
        info!(
            "  连接点{}: {:?} 距离站点: {}",
            i, connection_point, dist_to_connection
        );
    }
}
