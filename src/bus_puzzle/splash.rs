...
impl Plugin for SplashPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(OnEnter(GameStateEnum::Splash), spawn_splash_screen);
        ...
        app.add_systems(OnEnter(GameStateEnum::Splash), insert_splash_timer);
        ...
        app.add_systems(
            Update,
            enter_title_screen
                .run_if(input_just_pressed(KeyCode::Escape).and(in_state(GameStateEnum::Splash))),
        );
    }
}
...
fn check_splash_timer(
    ...
    mut next_screen: ResMut<NextState<GameStateEnum>>,
) {
    if timer.0.just_finished() {
        next_screen.set(GameStateEnum::MainMenu);
    }
}

fn enter_title_screen(mut next_screen: ResMut<NextState<GameStateEnum>>) {
    next_screen.set(GameStateEnum::MainMenu);
}
