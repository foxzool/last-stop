...
// 使用相对路径引用同模块下的其他文件
use super::{
    GridPos, RouteSegmentType, LevelData, AvailableSegment, ObjectiveType,
    ObjectiveCondition, PassengerColor, AgentState, PathfindingAgent,
    LevelManager, GameState, GameScore, world_to_grid,
    SegmentPlacedEvent, SegmentRemovedEvent, ObjectiveCompletedEvent,
    LevelCompletedEvent, InventoryUpdatedEvent, PathNode
};
...
fn handle_camera_controls(
    ...
    for wheel_event in mouse_wheel_events.read() {
        ...
        camera_controller.zoom = camera_controller.zoom
    ...
    if movement != Vec3::ZERO {
        ...
        camera_transform.translation += movement * camera_controller.pan_speed * dt * camera_controller.zoom;
    }
}
...
fn handle_segment_placement(
    ...
    if mouse_button_input.just_pressed(MouseButton::Left) {
        if let (Some(segment_type), Some(grid_pos)) =
            (input_state.selected_segment.clone(), input_state.grid_cursor_pos) {
            ...
            if is_valid_placement(&game_state, grid_pos, &segment_type) {
                if let Some(&available_count) = game_state.player_inventory.get(&segment_type) {
                    if available_count > 0 {
                        ...
                        game_state.placed_segments.insert(grid_pos, PlacedSegment {
    ...
    if keyboard_input.just_pressed(KeyCode::Delete) || keyboard_input.just_pressed(KeyCode::KeyX) {
        if let Some(grid_pos) = input_state.grid_cursor_pos {
            if let Some(placed_segment) = game_state.placed_segments.remove(&grid_pos) {
                ...
                *game_state.player_inventory.entry(placed_segment.segment_type.clone()).or_insert(0) += 1;
...
fn update_grid_preview(
    ...
    if let (Some(segment_type), Some(grid_pos)) =
        ...
        let world_pos = if let Some(level_data) = &game_state.current_level {
            grid_pos.to_world_pos(level_manager.tile_size, level_data.grid_size.0, level_data.grid_size.1)
        } else {
        ...
        commands.spawn((
            SpriteBundle {
                texture: asset_server.load(get_segment_texture_path(&segment_type)),
                transform: Transform::from_translation(world_pos + Vec3::Z * 0.1),
...
fn update_objectives(
    ...
    if game_state.objectives_completed.len() < objectives_len {
        game_state.objectives_completed.resize(objectives_len, false);
    }
...
fn handle_level_completion(
    ...
    if let Some(_level_data) = &game_state.current_level {
        let all_completed = game_state.objectives_completed.iter().all(|&completed| completed);
...
fn is_valid_placement(
    ...
    if let Some(level_data) = &game_state.current_level {
        ...
        if position.x < 0 || position.y < 0 ||
            position.x >= width as i32 || position.y >= height as i32 {
            return false;
        }
    }
...
fn spawn_route_segment(
    ...
    let world_pos = if let Some(level_data) = &level_manager.current_level {
        position.to_world_pos(level_manager.tile_size, level_data.grid_size.0, level_data.grid_size.1)
    } else {
    ...
    commands.spawn((
        SpriteBundle {
            ...
            transform: Transform::from_translation(world_pos + Vec3::Z * 0.5)
                .with_rotation(Quat::from_rotation_z((rotation as f32) * std::f32::consts::PI / 180.0)),
            ..default()
        },
        super::RouteSegment {
            grid_pos: position,
...
fn check_objective_completion(
    ...
    match &objective.condition_type {
        ObjectiveType::ConnectAllPassengers => {
            let all_arrived = passengers.iter()
        ...
        ObjectiveType::MaxCost(max_cost) => {
            game_state.total_cost <= *max_cost
        },
        ObjectiveType::TimeLimit(time_limit) => {
...
fn calculate_network_efficiency(
    ...
    let total_travel_time: f32 = passengers.iter()
        .filter(|agent| matches!(agent.state, AgentState::Arrived))
...
fn calculate_passenger_satisfaction(passengers: &Query<&PathfindingAgent>) -> f32 {
    ...
    let satisfied_count = passengers.iter()
        .filter(|agent| matches!(agent.state, AgentState::Arrived))
        .count();
...
fn update_mouse_world_position(
    ...
    if let Some(cursor_pos) = window.cursor_position() {
        if let Some(world_pos) = camera.viewport_to_world_2d(camera_transform, cursor_pos) {
            ...
            input_state.grid_cursor_pos = Some(GridPos::new(grid_x, grid_y));
        }
    }
}
...
fn handle_segment_placement(
    ...
    mut segment_placed_events: EventWriter<SegmentPlacedEvent>,
    mut inventory_updated_events: EventWriter<InventoryUpdatedEvent>,
    level_manager: Res<crate::LevelManager>,
) {
    if mouse_button_input.just_pressed(MouseButton::Left) {
        if let (Some(segment_type), Some(grid_pos)) =
            (input_state.selected_segment.clone(), input_state.grid_cursor_pos) {
...
fn handle_segment_rotation(
    mut game_state: ResMut<GameState>,
    mut route_segments: Query<&mut Transform, With<crate::RouteSegment>>,
    mouse_button_input: Res<ButtonInput<MouseButton>>,
    input_state: Res<InputState>,
) {
    if mouse_button_input.just_pressed(MouseButton::Right) {
...
fn is_valid_placement(
    ...
    if let Some(level_data) = &game_state.current_level {
        if let Some(terrain_type) = level_data.terrain.get(&position) {
            match terrain_type {
                crate::TerrainType::Building => return false,
                crate::TerrainType::Water => {
                    return matches!(segment_type, RouteSegmentType::Bridge);
                },
                crate::TerrainType::Mountain => {
                    return matches!(segment_type, RouteSegmentType::Tunnel);
                },
                _ => {}
            }
...
fn spawn_route_segment(
    ...
    segment_type: RouteSegmentType,
    rotation: u32,
    level_manager: &crate::LevelManager,
) -> Entity {
    let world_pos = if let Some(level_data) = &level_manager.current_level {
        position.to_world_pos(level_manager.tile_size, level_data.grid_size.0, level_data.grid_size.1)
    } else {
...
fn count_transfers_in_path(path: &[crate::PathNode]) -> u32 {
    let mut transfers = 0;
    let mut current_route_id = None;
