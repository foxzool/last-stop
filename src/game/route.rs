use crate::game::{
    grid::{GridPos, RouteSegmentType},
    level::{LevelManager, PassengerEntity, RouteSegment, StationEntity},
...
fn find_paths_for_new_passengers(
    ...
    mut passengers: Query<
        ...
        (Added<PathfindingAgent>, Without<PassengerEntity>),
    >,
) {
...
fn update_passenger_movement(
    ...
    let Some(level_data) = &level_manager.current_level else {
        return;
    };
        ...
        match agent.state {
            ...
            AgentState::Traveling => {
                if agent.current_step < agent.current_path.len() {
                    ...
                    let target_pos = level_data.grid_to_world(current_node.position).extend(0.0);
